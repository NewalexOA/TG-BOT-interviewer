...
from backend import (create_user_table, register_user, get_random_question,
                     update_user_stats, check_answer_with_openai)
...
async def handle_answer(message: Message):
    ...
    if telegram_id in bot_data:
        question = bot_data[telegram_id]
        correctness, explanation = check_answer_with_openai(question, user_answer)
        await message.answer(f"{correctness}\n{explanation}")
        update_user_stats(telegram_id, correctness.lower() == "правильно")
